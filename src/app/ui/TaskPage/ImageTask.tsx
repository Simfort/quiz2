...
export default function StartTask({ data }: { data: Task }) {
  ...
  return (
    <section className="flex w-[476px] max-lg:w-[300px] items-center flex-col gap-[16px]">
      ...
      <ul className="flex flex-col items-center gap-5 w-1/1">
        {data.answers?.map((val, i) => (
          <button
            ...
            } w-1/1 flex gap-14 rounded-[0.85rem] cursor-pointer p-[1rem] font-bold items-center`}
            key={i}>
            <div className="w-[75px] h-[45px] rounded-[0.5rem] flex items-center justify-center   shadow-[rgba(0,2,18,0.41)_2px_2px_8px]">
              <Image
